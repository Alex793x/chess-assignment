...
public class Board {
    ...
    private static Move parseMove(String moveInput, Board board) {
        String[] parts = moveInput.split(" ");
        if (parts.length != 2) {
            return null;
        }

        int fromSquare = convertSquare(parts[0]);
        int toSquare = convertSquare(parts[1]);
        ...
        return new Move(fromSquare, toSquare, 0, piece, capturedPiece, isCapture, false, board.getHalfMoveClock(), false, false, 0, null);
    }

    private static int convertSquare(String square) {
        if (square.length() != 2) {
            return -1;
        }
    ...
    public static void main(String[] args) {
        ...
        while (!board.isGameOver()) {
            if ((board.getCurrentPlayer() == CurrentPlayer.WHITE && isPlayerWhite) ||
                ...
                Move playerMove = parseMove(moveInput, board);
