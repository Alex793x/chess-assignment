    ...
    public List<Integer> generateMovesForPawn(int square, PieceColor color) {
        ...
        if (isSquareEmpty(oneStepForward, allOccupancies)) {
            ...
            if (((color == PieceColor.WHITE && (square / 8) == 1) ||
                    ...
                    isSquareEmpty(twoStepsForward, allOccupancies)) {
                moves.add(twoStepsForward);
            }
        }

        // Capture moves to the left and right
        if (isWithinBoardBounds(leftCapture) && isSquareOccupiedByEnemy(leftCapture, enemyOccupancies)) {
            moves.add(leftCapture);
        }
        if (isWithinBoardBounds(rightCapture) && isSquareOccupiedByEnemy(rightCapture, enemyOccupancies)) {
